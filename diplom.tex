...
\newcommand{\pProtVar}{p} % Protocol variant
\newcommand{\pNCMode}{a} % Network coding mode
\newcommand{\pEncMods}{b} % Number of encryption modules
\newcommand{\pAuthMods}{c} % Number of authentication modules
\newcommand{\pRQSize}{rs} % Queue size limit for router←→router input queues
\newcommand{\pARQLimit}{d} % ARQ Limit
\newcommand{\pARQTimeout}{e} % ARQ flit inter-arrival timeout
\newcommand{\pRStrat}{f} % Routing strategy
\newcommand{\pNumAttackers}{g} % Number of malicious routers
\newcommand{\pAttackProb}{h} % Probability of an attack
...
\begin{document}
	...
	\chapter*{Task} % TODO: include scan of printed task with signature here
    Lorem ipsum
	
	\chapter*{Declaration Of Academic Integrity} % TODO: gibt es eine Regelung vom Prüfungsamt?
    I hereby declare that the present diploma thesis entitled
    ...
    was written entirely by myself. I have listed all the used sources and means and clearly marked quotations as such.

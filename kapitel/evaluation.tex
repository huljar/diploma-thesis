...
workaround, a hard queue size limit is only implemented for \gls{dor}. For the other strategies, congestion signals are still sent out once a queue
...
higher value that is used after an \gls{arq} was issued to await the answer (the post-\gls{arq} timeout). As they are interdependent, only the former
is empirically determined. The latter is calculated from the inter-arrival timeout and the Manhattan distance between the
two affected communication partners. More precisely, if $t_1$ is the inter-arrival timeout, $t_2(S, D)$ is the post-\gls{arq} timeout for a
...
Hence, $t_1$ needs to be determined through experiments. Since this is the first parameter to be fixed, the other input values for the simulation
are estimated. The tests are for the most part independent of the protocol variant as the same injection rate is used for all of them. Table
\vref{tab:setuparqtimeouts} presents how the simulator is set up.
...
This also means that the sizes of the queues in front of the crypto modules (cf. Figure \ref{fig:omnetni}) do not need to exceed around 10 flits.
...
\Gls{arq} flits and retransmissions are not encompassed in the base network injection rate (see Section \ref{sec:environmenteval}). However, the total injection rate including 

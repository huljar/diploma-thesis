...
A core aspect of this thesis is the exploration of the different routing strategies and their effects on the performance and reliability of
...
paths at disposal. The path selection methods described in this chapter thus only apply to those source-destination node pairs lying on different
...
Routing strategies are classified according to their properties. The following list and Figure \ref{fig:routingcharacteristics} explain the important
characteristics and the distinctions between them.
\begin{itemize}
    \item \textbf{Oblivious vs. adaptive.} When the source or an intermediate node decides on the next hop of the route, it may take the current
        network state into account. For instance, a congestion detected on the northern port may dissuade the router from forwarding the flit in this
        direction and instead use a different port, as long as the resulting route is still minimal. With this adaptive behaviour, the path is
        influenced by the network state and thus a different path may be chosen at a later point in time. In contrast, an oblivious strategy does not
        take the network state into consideration; in case of a congestion, forwarding of the flit is delayed until it is resolved. The oblivious
        strategies explored here are \gls{dor} (Section \ref{subsec:dor}) and \gls{romm} (Section \ref{subsec:romm}), while \gls{dm} (Section
        \ref{subsec:dm}) and \gls{ramm} (Section \ref{subsec:ramm}) are adaptive.
    \item \textbf{Static vs. dynamic.} Static path selection implies that exactly one fixed, deterministically chosen path exists for each
        pair of source and destination node. A dynamic strategy entails a path being selected for each flit individually at runtime. Consequently,
        static strategies are always oblivious and adaptive ones are always dynamic. Since the emphasis lies on the usage of multiple paths, the only
        investigated static strategy is \gls{dor}, serving as a baseline for comparison with the other ones.
\end{itemize}
\vspace{0.5\baselineskip}
...
\begin{figure}
    ...
    \caption[Routing strategies by category]{Various routing strategies categorized by their characteristics.}
...
this scenario with the network coded interwoven authentication protocol (G2C3).
...
\begin{figure}
    ...
    the paths of flits 2 and 3 to not include an attacker, enabling the destination node to successfully decode the generation. With \gls{dor}, every
    flit would use path 1, enabling the malicious router to corrupt all of them.}
    \label{fig:circumventattackers}
\end{figure}

Furthermore, adaptive strategies may enable routers to circumvent congestions and entail a more uniform distribution of the traffic over the network.

Mention the perceived/envisioned advantages of dynamic routing (different paths for each flit within the same sender-receiver pair → thanks
to NC enough flits will arrive hopefully even if one path contains a corrupted router).

Adaptive: take network state into account at each intermediate router when deciding the next hop. Mention that routers have input queues which may
become full → details in Chapter 6. Adaptivity: routers are aware of the neighboring router's input queue status, try to use a path where the queue is
not full

\subsection{Dimension Ordered}\label{subsec:dor}
\textit{Dimension Order Routing}
...
\subsection{Dynamic Minimal}\label{subsec:dm}

\subsection{Randomized Oblivious Multi-Phase Minimal}\label{subsec:romm}

\subsection{Randomized Adaptive Multi-Phase Minimal}\label{subsec:ramm}

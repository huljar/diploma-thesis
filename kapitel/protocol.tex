...
in a network coded setting.% TODO: remove/replace this shitty wikipedia reference
...
% TODO: here: ECB is used. This is not secure, e.g. because same plaintexts yield same ciphertexts. One possibility: CTR mode where the flit ID is the
% counter. As ID is unique per encrypted flit, it can be used as the counter; this way, decryption also works when the flit stream is interrupted (no
...
\[
    ...
    \end{split}
\]
Each row of the result matrix represents one combination, with $\beta_i = (\beta_{i,\,0}, …, \beta_{i,\,G-1})\ |\ i \in [0, C-1]$ standing
...
% TODO: mention that G2C4 is better than e.g. sending every flit twice (which also results in a 100% flit increase)
...
% TODO: explain case of complete transmission unit lost; receivers can detect this through the ID stream from this sender not being continuous.
% full transmission unit loss beyond scope of this thesis though
...
\begin{itemize}
    \item \textbf{Oblivious vs. adaptive.} When the source or an intermediate node decides on the next hop of a route, it may take the current
        network state into account. For instance, a congestion detected on the preferred output port\footnote{Congestion is detected by inspecting the status
        ...
        until the path is available again.
...
\end{figure}
% TODO: explain the envisioned advantages of RAMM a bit more (having an intermediate node may enable circumvention of larger congested ares, which DM
% by itself may be unable to do)
% TODO: table comparing the routing strategies (adaptive yes/no, static yes/no, out-of-order arrival possible yes/no, ...) (maybe no table but mention

...
% TODO mention that ARQs are not encrypted because they only contain metadata, not NC'd because there is only one ARQ flit and unknown when the next
% ARQ with same destination will be issued → infeasible to create a generation, not auth'd to save time → what attack scenarios are feasible then?
% attackers may flood the network with ARQs and trigger unnecessary retransmissions, but if attacker is able to inject arbitrary ARQs he can flood the
% network anyway. Modified ARQ may make the RT send out other unrequested flits instead, we simplify this case to simply ignoring the ARQ. In
% practive, RT doesn't know if ARQ was modified due to no auth

...
to the corresponding network interface takes one cycle. In a hardware implementation, this is facilitated by having parallel communication channels
that can transmit all bits of a flit at the same time. To do so, one wire is required at the physical layer for each bit to be transmitted in
parallel, resulting in a channel width of at least 141 bit for uncoded flits and 149 bit for network coded flits.

...
\begin{figure}
    ...
    \caption[short]{The processing of a transmission unit for network coded individual authentication (G2C3) from the outside.}
...
This amounts to the following procedure for sending flits to the network:
\begin{enumerate}
...
The following processing steps are performed for flits arriving from the network:
\begin{enumerate}
    ...
    \item \textbf{Verification.} This step is similar to its counterpart in uncoded individual authentication, but not exactly the same. As there are
        multiple verifications required for each transmission unit (one per combination), all of them have to succeed. Otherwise, both of the decoded
...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{timeout-indauth-nc}
    \caption[short]{long}
...
them together, the mode field is used to distinguish them (by setting different values for the first and second split, respectively).

A normal \gls{mac}, as used in individual authentication, has a size of 64 bit by design. Thus, the authentication scheme has to be adapted. This is
where the concept of \textit{authcodes} comes in (cite authcode paper here). The key idea is to first compute a \gls{mac} as before, but then use it
as a key to create an authcode of half its size (i.e., 32 bit -- precisely the number of available bits).
% TODO: how are authcodes secure? if an attacker has a chance of 0.5 to flip a bit without this being detected?

to having a dedicated, separate \gls{mac} flit. However, since the payload size is still 64 bit, it cannot hold 

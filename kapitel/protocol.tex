...
Over the course of this chapter, the design of the communication protocol in all variants will be thorougly explained. To begin, necessary definitions and fundamental
...
\section{Definitions}
Mathematical definitions here: C = e(P, K), MAC = e(P, K2), ...
...
\begin{itemize}
    \item \textbf{Parallel modules.} As the cryptographic computations are the operations with the highest latency in the network interfaces, they
        ...
        instance, each network interface could have three encryption modules, allowing it to encrypt up to three flits simultaneously.
    \item \textbf{Hardware-optimized ciphers.} In Section \ref{sec:lightweightcrypto}, it was already mentioned that conventional block ciphers, such
        ...
        compared \cite{harttung17lightweightcrypto}. The most promising algorithm is PRINCE \cite{borghoff12prince}: it encrypts and
        decrypts the fastest and has a competitive area requirement\footnote{For this thesis, low latency is deemed slightly more important than low chip area
        usage. Should area become the more important factor, other algorithms (such as PRESENT \cite{bogdanov07present}) might be better suited.}.
        Furthermore, with a key length of 128 bit, it provides relatively high security\footnote{Similar algorithms often have a key length of 80 or
        96 bit \cite[5]{harttung17lightweightcrypto}.}. To date, it remains unbroken depite monetary incentives \cite{princechallenge}.
        More details on PRINCE can be found in Section (insert ref here).
...
security of the scheme. There are three general approaches to bring them together:
\begin{itemize}
    \item \textbf{Encrypt-and-MAC.}  \cite{bellare00authenc}
...
With a size of 141 or 149 bit, respectively, flits are relatively small. While this entails each one to carry comparatively little

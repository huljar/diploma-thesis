...
On the side of the receiver, the combinations are decoded to recover the original flits. At least $G$ out of the $C$ combinations must arrive at their
...
Structure of ARQ: same header fields as uncoded flits in Section \ref{subsec:flitstructure}, mode field set to <arq>. Payload contains information on what
needs to be retransmitted. For UC: payload contains the missing modes. For NC: FID field contains GID and payload contains the missing modes for each
GEV of the generation. If not all GEVs are known by the receiver (timeout), then payload contains the GEV/mode pairs of what was received, and the
sender has the responsibility to look up the missing ones.

The number of allowed ARQs is limited per transmission unit. If ARQ is required but the limit is already reached → error, transmission failed.

When handling incoming flits from the network, there are two cases that trigger an \gls{arq} issuance. The first case is an integrity check
failing as described above. The second case occurs when the temporal gap between the arrival of flits from the same transmission unit becomes too
large. As the first flit from a particular unit arrives, a countdown timer is started that is associated with this unit. Upon receiving further
flits belonging to the same unit, the timer is reset. Should it reach zero (i.e., a timeout occurs), the remaining flits from this transmission unit
are assumed lost and an \gls{arq} is issued requesting their retransmission. 
...
inter-arrival timout described above. Furthermore, the value depends on the position of the affected sender node in the network: it increases
proportinally with the manhattan distance between the communication partners. This is done to account for the round trip time of the \gls{arq} and its
answer.
...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{protocol-chart-indauth}
    \caption[short]{long}
    \label{fig:protchartindauth}
\end{figure}

The sequence of actions for sending flits to the network (upper part of Figure \ref{fig:protchartindauth}) is as follows:
\begin{enumerate}
...
\begin{figure}
    ...
    \caption[short]{CBC-MAC. This corresponds to the function $a_k(f)$ in Section \ref{subsec:crypto}.}% TODO
...
vice versa). An examplary sequence of events involving \glspl{arq} is depicted in Figure \vref{fig:singletimeoutindauthuc}. The exact timeout values
are discussed in Chapter \ref{ch:implementation}.

\begin{figure}
    \centering
    \includegraphics[width=0.7\textwidth]{single-timeout-indauth-uc}
    \caption[short]{long}
...
It is desirable to send out as little \glspl{arq} as necessary, as pointed out in Section \ref{subsec:arqretransmissions}. Furthermore, with a limited
...
middle ground, the issuance of \glspl{arq} is only postponed if another flit from the same transmission unit is currently being verified\footnote{More
...
\gls{arq} should the integrity check fail.

\subsection{Interwoven Authentication}\label{subsec:intauth}
\subsubsection{Uncoded Variant}
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{protocol-chart-intauth}
    \caption[short]{long}
...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{protocol-chart-genauth-nc}
    \caption[short]{long}

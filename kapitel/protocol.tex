...
generations, consisting of three or more flits, are also feasible, but have proven to entail higher latencies \cite[2]{moriam18activeattackers}, which
...
where $y = (\beta_{encoded} \| p_{encoded})$ denote the encoded \glspl{gev} and payloads, $\alpha$ represents the randomly selected coefficients, and
$x = (\beta \| p)$ stands for the original \glspl{gev} (i.e., the unit vectors) and payloads. The other header fields for the encoded flits are
adopted unaltered from the original flits.

For this thesis, variants with $C = 3$ and $C = 4$ combinations per generation are investigated; these schemes are referred to as \textit{G2C3} and
\textit{G2C4}, respectively.

On the side of the receiver, the combinations are decoded to obtain the original flits. For this process to succeed, at least $G$ of the $C$
combinations need to arrive at their destination\footnote{In fact, $G$ linearly independent combinations are required. Fortunately, they fulfill this
...
the linear equation system that is established

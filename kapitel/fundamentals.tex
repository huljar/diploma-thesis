...
former is attained by optimizing the components for low area requirements and power consumption, while the latter is realized % TODO: attained
...
algorithms and single-cycle routing are pursued to meet these criteria. They are explained in detail in Sections (insert refs here).% TODO: refs to relevant sections with details
...
In addition, it contains a payload that carries the actual information to be transmitted. % TODO: mention that flits are the atomic logical unit that
%can be sent in a single cycle
